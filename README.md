...
В архитектуру заложена возможность в будущем записывать другие типы данных, на данный момент только float. Предусмотрена иерархия пользователей:
...
метрикам доступ других пользователей, при этом пользователь может быть только класса spectator. Класс spectator предназначен только для 
...
- duration: время хранения истории метрики в днях
...
Для работы со скриптами требуется перейти в директорию bash:
```commandline
cd bash
```
...
users/add.sh 0 maks simple_pass
```
